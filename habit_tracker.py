...
class Habit_Tracker():
    def __init__(self, db):
        ...
        self.cur.execute("""CREATE TABLE IF NOT EXISTS
        habits(habit_id INTEGER PRIMARY KEY, user_id INTEGER, name TEXT, frequency INTEGER, state INTEGER, streak INTEGER, history INTEGER, longest_streak INTEGER,last_done TEXT, last_check TEXT, fulfilled INTEGER, failed INTEGER, FOREIGN KEY(user_id) REFERENCES user(user_id))""")
    ...
    def create_habit(self, name = "", frequency = None):
        """
        ...
        while not_done:
            ...
            num += 1
            not_done = self.add_habit_db(num, name, frequency)
        habit = Habit(num, name, frequency, False, 0, [0, 0, 0, 0, 0], 0, datetime.datetime.now().date(),
                      datetime.datetime.now().date(), 0, 0)
        self.habit_list.insert(num - 1, habit)
        print("your habit has been added to your list")
        time.sleep(0.5)

    def add_habit_db(self, num, name, frequency):
        """
        ...
        try:
            id = num
            self.cur.execute(
                f"INSERT INTO habits VALUES ({id},{self.id_list[0]},'{name}', {frequency}, 0, 0, 00000, 0, datetime('now'), datetime('now'), 0,0)")
            self.conn.commit()
            return False
        except:
            return True
    ...
    def menu(self):
        """
        menu that shows all possible actions
        """
        self.update_habits()
        x = input(
    ...
    def load_habits(self):
        ...
        for row in data:
            ...
            history = [int(digit) for digit in str(row[6])]
            ...
            habit = Habit(row[0], row[2], row[3], state, row[5], history, row[7], last_done, last_check, row[10],
                          row[11])
